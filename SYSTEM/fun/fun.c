...
signed char keytim[25]={0};  	//°´¼üÑÓÊ±--È¥¶¶
u8  candata[8];
u8  KeyBYTE[8]={0};
u16  temp1,temp5;
...
// ¶¨Òå³£Á¿
//#define DEBOUNCE_TIME_US 200 // È¥¶¶¶¯Ê±¼äÉèÖÃÎª200Î¢Ãë
...
//	rgbw2814[0]=1;rgbw2814[1]=2;rgbw2814[2]=3;  rgbw2814[2]=3;rgbw2814[9]=3;rgbw2814[24]=3;
	...
	{ 
		rgbwtim=0;				// ÖØÖÃ¼ÆÊ±Æ÷±äÁ¿rgbwtimÎª0
		if(RGBWon==37)			//ÊÇ·ñ¿ªÆôRGBW
		{
			RGBWon=0;			// ½«RGBWonÖØÖÃÎª0
			rgbwtim=0;			// ÖØÖÃ¼ÆÊ±Æ÷±äÁ¿rgbwtimÎª0
			WS2812B_Task();  	//RGBW°´¼ü
		}
	}
...
//ÉÏÎ»»ú´«ÈëµÄÊý¾ÝÖ±½Ó·¢ËÍµ½´ÓÕ¾£¬½ØÈ¡Êý¾ÝÊ±Tx2BufÒª·¢ËÍµ½´ÓÕ¾µÄÊý¾Ý
	OLEDtinct(Rx2Buf,rgbw2814,0,25);	//½ØÈ¡RGBÏÔÊ¾Êý¾Ý
	temp1++;
...
//====================·¢ËÍOLEDÏÔÊ¾Êý¾ÝÃüÁî====================================
//485´®¿Ú·¢ËÍÊý¾Ýµ½´Ó»ú£¬·¢ËÍOLEDÏÔÊ¾ÆÁÊý¾Ý -- !!!Ä¿Ç°Ã»ÓÃµ½
void USART3_RD(void)
{
/*
	R485_1=1;  
...
//typedef enum {
//	KEY_IDLE,    // °´¼ü¿ÕÏÐ×´Ì¬
//	KEY_PRESSED  // °´¼üÒÑ°´ÏÂ×´Ì¬
//} KeyState;
//volatile KeyState keyState = KEY_IDLE;  // µ±Ç°°´¼ü×´Ì¬
//volatile uint32_t lastPressTime = 0;    // ÉÏ´Î°´¼üÊ±¼ä£¨¼ÆÊýÆ÷Öµ£©
//volatile uint8_t lastKeyState = 0;      // ÉÏÒ»´Î°´¼ü×´Ì¬

void key(void)
{
//test key one send date?
//	uint32_t currentTime = TIM2->CNT; 		// »ñÈ¡µ±Ç°¶¨Ê±Æ÷¼ÆÊýÖµ
//	
...
//			KeyBYTE[0]&=~(1<<0);
//			keyState = KEY_PRESSED; // ¸üÐÂ°´¼ü×´Ì¬ÎªÒÑ°´ÏÂ
//		}
//    } else if(!key1){
//		keyState = KEY_IDLE; // °´¼üÊÍ·Å£¬¸üÐÂ×´Ì¬Îª¿ÕÏÐ
//	}
