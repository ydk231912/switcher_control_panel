...
public:
    void init(){
        ...
        data_frame.reset(new DataFrame(control_panel_config));
        data_frame->switcher = switcher;

        main_controller.reset(new MainController(*uart_port1_config,*uart_port2_config, control_panel_config ,data_frame));
        main_controller->switcher = switcher;
    ...
    void start(){
        can_frame_processor->on_key_press = [&] (const std::string &k) {
            main_controller->handle_key_press(k);
        };
    ...
    void activate_config_update_notify(){
        switcher->store_screens_config(screen_frame_hex_data, switcher->pgm_commands, switcher->pvw_commands);
