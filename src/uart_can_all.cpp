...
public:
  // 构造函数，初始化 CAN 套接字
  CanSocket(const std::string &interface) : interface_name(interface) { setupSocket(); }
...
private:
  ...
  {
    logger->info("receive can frame_id : {}", frame.can_id);
    switch (frame.can_id)
    {
    case FRAME1_ID:
  ...
  void handle_FrameData1(const can_frame &frame, int frame_id)
  {
    ...
    if (frame.data[0] != 0x00){
      ...
      if (idx == 17){
        ...
        // {
          logger->info("receive can dsk : {}", idx);
          switcher->dsk(0);
        // }
      }
      else if (idx == 19){
  ...
  void handle_FrameData3(const can_frame &frame, int frame_id)
  {
    ...
    if (frame.data[0] != 0x00){
      ...
      printFrameData(frame);
      logger->info("receive can pgm : {}", this->pgm);
    }
    else if (frame.data[1] != 0x00){
      idx = calculateidex(frame.data[1], 9);
      if (idx < 12){
        ...
        printFrameData(frame);
        logger->info("receive can pvw : {}", this->pvw);
      }
    }
    else if (frame.data[2] != 0x00)
    {
  ...
  {
    logger->info("Frame from {}: ID: {}  DLC: {}  Data: ", label,  frame.can_id, static_cast<int>(frame.can_dlc));
...
int main(int argc, char **argv) {
  ...
  //   // 创建一个多输出的日志器，将日志同时写入控制台和文件
  //   auto logger = std::make_shared<spdlog::logger>("multi_sink_logger", spdlog::sinks_init_list({console_sink, file_sink}));
  ...
  try {
    ...
    switcher->set_dsk_status_handler(
        std::bind(&MainController::handler_dsk_status, &controller,
                  std::placeholders::_1));
    switcher->set_transition_type_handler(
        std::bind(&MainController::handler_transition_type, &controller,
                  std::placeholders::_1));
