...
int st_app_video_get_lcore(struct st_app_context *ctx, int sch_idx, bool rtp,
  ...
  if (rtp) {
    if (ctx->rtp_lcore[sch_idx] < 0) {
      ...
      logger->info("{}, new rtp lcore {} for sch idx {}", __func__, video_lcore,
                   sch_idx);
    }
  } else {
    if (ctx->lcore[sch_idx] < 0) {
      ...
      logger->info("{}, new lcore {} for sch idx {}", __func__, video_lcore,
                   sch_idx);
    }
  }
...
static void user_param_init(struct st_app_context *ctx,
...
#else
    ctx.params.xdp_info[i].queue_count =
        max_tx_queue_count + max_rx_queue_count;
#endif
  }
...
static void init_decklink(st_app_context *ctx) {
  ...
  for (auto &device : manager.get_device_status()) {
    logger->info("decklink index={} display_name={} persistent_id={} "
                 "device_label={} support_format_dection={}",
                 device.index, device.display_name, device.persistent_id,
                 device.device_label, device.support_format_dection);
  }
...
int main(int argc, char **argv) {
  ...
  if (ret < 0) {
    logger->error("{}, session count invalid, pass the restriction {}",
                  __func__, ST_APP_MAX_RX_VIDEO_SESSIONS);
    return -EINVAL;
  }
