...
static int create_display_context(struct st_display* d) {
  ...
  d->window =
      SDL_CreateWindow(title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
                       d->window_w, d->window_h, SDL_WINDOW_OPENGL);
...
static void* display_thread_func(void* arg) {
  struct st_display* d = (st_display*)arg;
  int idx = d->idx;
  ...
  //#ifdef WINDOWSENV
    ...
    if (ret < 0) {
      // logger->error("{}, create display context fail: {}", __func__, idx);
...
int st_app_uinit_display(struct st_display* d) {
  ...
  if (d->display_thread) {
    ...
    st_pthread_mutex_unlock(&d->display_wake_mutex);
    // logger->info("{}({}), wait display thread stop\n", __func__, idx);
...
//   logger->error("{}({}),APP_HAS_SDL2_TTF", __func__, idx);
#endif
  if (d->fmt == SDL_PIXELFORMAT_UYVY) {
    d->front_frame_size = width * height * 2;
  } else {
    // logger->error("{}, unsupported pixel format {}", __func__, idx);
  ...
  if (!d->front_frame) {
    // logger->error("{}, alloc front frame fail", __func__);
  ...
  //#ifndef WINDOWSENV
    ...
    if (ret < 0) {
      // logger->error("{}, create display context fail {}", __func__, ret);
      // //err("%s, create display context fail: %d\n", __func__, ret);
  ...
  if (ret < 0) {
    ...
    // logger->error("{}({}), create display thread fail {}", __func__, idx, ret);
