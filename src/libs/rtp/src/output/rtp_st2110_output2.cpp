...
#include "output/rtp_st2110_output2.h"

using namespace seeder::core;
namespace seeder::rtp
{
    rtp_st2110_output2::rtp_st2110_output2(rtp_context& context)
    :context_(context)
    ,udp_sender_(std::make_unique<net::udp_sender>())
    ,udp_sender2_(std::make_unique<net::udp_sender>())
    ...
    rtp_st2110_output2::~rtp_st2110_output2()
    {
    }
     ...
     * 
     */
    void rtp_st2110_output2::encode()
    {
    ...
    void rtp_st2110_output2::do_encode()
    {
     ...
     * 
     */
    void rtp_st2110_output2::send_packet()
    {
        udp_thread_ = std::make_unique<std::thread>(std::thread([&](){
            ...
            packet_drain_duration = packet_drain_duration * 2;
            while(!abort)
            {
                try
                {
                    ...
                    if(packet)
                    {
                        ...
                        if(packet->is_first_of_frame())
                        {
                            ...
                            timestampe_ = static_cast<uint32_t>(video_time % RTP_WRAP_AROUND);
                            frame_number_++;
                        }
                        ...
                        packet->set_timestamp(timestampe_);
    ...
    void rtp_st2110_output2::send_packet2()
    {
        udp_thread2_ = std::make_unique<std::thread>(std::thread([&](){
            // bind thread to one fixed cpu core
            // cpu_set_t mask;
            // CPU_ZERO(&mask);
            // CPU_SET(23, &mask);
            ...
            while(!abort)
            {
                try
                {
                    ...
                    if(packet)
                    {
                        ...
                        // send data synchronously
                        udp_sender2_->send_to(packet->get_data_ptr(), packet->get_data_size(), 
                                             context_.multicast_ip, context_.multicast_port);
     ...
     * 
     */
    void rtp_st2110_output2::set_frame(std::shared_ptr<core::frame> frm)
    {
     ...
     * 
     */
    std::shared_ptr<core::frame> rtp_st2110_output2::get_frame()
    {
     ...
     */
    std::shared_ptr<packet> rtp_st2110_output2::get_packet()
    {
        std::shared_ptr<rtp::packet> packet = nullptr;
     ...
     * 
     */
    void rtp_st2110_output2::set_packet(std::shared_ptr<packet> packet)
    {
        // std::unique_lock<std::mutex> lock(packet_mutex_);
        ...
        {
            auto ret = packet_bf2_.push(packet);
            qid_ = 0;
        }
     ...
     * @return int 
     */
    int rtp_st2110_output2::calc_packets_per_frame()
    {
        int packets = 0;
