...
volatile u8  ucByteBufRX2[DMARX2];    //串口2通讯数组
...
//	delay_ms(500);				//等待DMA时钟稳定
	...
	DMA_CHx->CMAR=(u32)cmar; 	//DMA2,存储器地址
	DMA1_MEM_LEN2=cndtr;      	//保存DMA传输数据量
	DMA_CHx->CNDTR=cndtr;    	//DMA2,传输数据量
	DMA_CHx->CCR=0X00000000;	//复位
	DMA_CHx->CCR|=1<<4;  		//从存储器读  
	DMA_CHx->CCR|=0<<5;  		//普通模式
	DMA_CHx->CCR|=0<<6; 		//外设地址非增量模式
...
void DMA_USART3_Tx_Config(DMA_Channel_TypeDef* DMA_CHx,u32 cpar,u32 cmar,u16 cndtr)
{
	...
	RCC->AHBENR|=1<<0;			//开启DMA1时钟
 //	delay_ms(500);				//等待DMA时钟稳定
	DMA_CHx->CPAR=cpar; 	 	//DMA3 外设地址 
	DMA_CHx->CMAR=(u32)cmar; 	//DMA3,存储器地址
	DMA1_MEM_LEN2=cndtr;      	//保存DMA传输数据量
	DMA_CHx->CNDTR=cndtr;    	//DMA3,传输数据量
	DMA_CHx->CCR=0X00000000;	//复位
	DMA_CHx->CCR|=1<<4;  		//从存储器读  
	DMA_CHx->CCR|=0<<5;  		//普通模式
	DMA_CHx->CCR|=0<<6; 		//外设地址非增量模式
